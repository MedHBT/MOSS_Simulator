 ...
 * @author Ray Ontko
 * CHANGELOG: 
 *   - add support for multiuser whithin the Kernel.initialize method
 *   - completed the tbd section whithin creat method ( check for users permission on created files )
 *   - completed the tbd section whithin open method ( check for users permission on opened files )
 */
public class Kernel
{
  ...
  /***************************************************************/
  /* Modification maded by Med Hassine Ben Taieb and Amine Haddad*/
  /***************************************************************/
   ...
   * File type mask
   */
  //we have to change short to char in S_IFMT and S_IFREG because short is a signed type.
  //when we cast short to integer the complement bit will be filled be 1
  //so the logical and will give wrong result.
  
  public static final char S_IFMT = (char)0170000 ;

  /**
   * Regular file
   */
  
  public static final char S_IFREG = (char)0100000 ;
  
  /**********************************************************************/
  /* End of modification maded by Med Hassine Ben Taieb and Amine Haddad*/
  /**********************************************************************/
    ...
    throws Exception
  {
    ...
    while( st.hasMoreTokens() )
    {
      ...
      if ( ! name.equals("") )
      {
        ...
        /**********************************************************************/
   	 	/* Modification made by Med Hassine Ben Taieb and Amine Haddad*/
   	 	/**********************************************************************/
        ...
        short owner = currIndexNode.getUid();//get the current user id from the inode
        short group = currIndexNode.getGid();//get the current group id from the inode
        short permission;
        //check if the running user match the current and set permission
        if( process.getUid() == owner ){
        	permission = S_IRUSR;
        }
        //check if the running group match the current and set permission
        else if( process.getGid() == group ){
        	permission = S_IRGRP;
        }
        //if any of the two verification is undone the default permission is set to others
        else
        	permission = S_IROTH;
        //check the access else return error
        if( ( currIndexNode.getMode() & permission ) != permission )
        {
        ...
        /**********************************************************************/
   	 	/* End of modification made by Med Hassine Ben Taieb and Amine Haddad*/
   	 	/**********************************************************************/
    ...
    if ( indexNodeNumber < 0 ){
      ...
      // in which it is to be created is not writable
    	...
    	short owner = prevIndexNode.getUid();//get the user id from the inode
    	short group = prevIndexNode.getGid();//get the group id from the inode
    	short permission;
    	//check if the running user match the current and set permission
    	if( process.getUid() == owner ){
    		permission = S_IWUSR;
    	...
    	//if any of the two verification is undone the default permission is set to others
    		...
    		permission = S_IWOTH;
    	//check the access else return error
    	if( ( prevIndexNode.getMode() & permission ) != permission ){
    		process.errno = EACCES;
    		return -1;
    	}

      //for new file
      short currentUmask = process.getUmask() ;//get the current user mask
      currIndexNode.setMode( (short) ( mode & ~currentUmask ) ) ;//for file(0666 & ~umask) and for directories(0777 & ~umask) 
      // set the running process to the owner 
      currIndexNode.setUid(process.getUid());//set the user id in the inode
      currIndexNode.setGid(process.getGid());//set the group id in the inode
...
// System.out.println( "dirname = " + dirname.toString() ) ;
    ...
    throws Exception
  {
    ...
    short currentUID = process.getUid() ;
    short currentGID = process.getGid() ;
    short owner = indexNode.getUid() ;
    short group = indexNode.getGid() ;
    short mode = indexNode.getMode() ;

    short userPermission = 0, groupPermission = 0, othersPermission = 0;
    //set the mask depend on the given mode
    if (flags == O_RDONLY){
    	userPermission = S_IRUSR ;
        groupPermission = S_IRGRP ;
        othersPermission = S_IROTH ;
    }
    else if(flags == O_WRONLY){
    	userPermission = S_IWUSR ;
        groupPermission = S_IWGRP ;
        othersPermission = S_IWOTH ;
    }
    else{
    	userPermission = (short) (S_IRUSR | S_IWUSR);
        groupPermission = (short) (S_IRGRP | S_IWGRP);
        othersPermission = (short) (S_IROTH | S_IWOTH);
    }
    ...
    // check if the running process is the owner of file
    if( currentUID == owner )
    {
      // check permission set properly
      if( (mode & userPermission)  != userPermission)
      {
        process.errno = EACCES;
        return -1 ;
      }
    }
    // check if the running process is the group of file
    else if( currentGID == group)
    {
      // check permission set properly
      if( (mode & groupPermission) != groupPermission )
      {
        process.errno = EACCES;
        return -1 ;
      }
    }
    // finally check if others has read permission bit set
    else
    {
      if( (mode & othersPermission) != othersPermission )
      {
        process.errno = EACCES;
        return -1 ;
      }
...
*/
  ...
  public static Logger passwd ;
  public static Logger group ;
  public static SyntxAlyz shadow ;
  public static SyntxAlyz gshadow ;
  ...
  public static void initialize() throws Exception
  {
    ...
    process.setUid((short)0);
    process.setGid((short)0);
    
    fd = open( "/etc/passwd",O_RDONLY);//open the password file
    passwd = new Logger(fd,":");//load file content
    close(fd);//close file
    
    fd = open( "/etc/group",O_RDONLY);//open the group file
    group = new Logger(fd,":");//load file content
    close(fd);//close file
    
    fd = open("/etc/shadow",O_RDONLY);//open the shadow file
    shadow = new SyntxAlyz(fd,":");//load file content
    close(fd);//close file
    
    fd = open("/etc/gshadow",O_RDONLY);//open the gshadow file
    gshadow = new SyntxAlyz(fd,":");//load file content
    close(fd);//close file
    
    process.setUid(uid) ;
    process.setGid(gid) ;
    
    // uid does not match any
    if( !passwd.belongs( uid ) )
    {
      System.out.println( uid + " does not belong to any user in password file.") ;
      exit(1);
    }
    // gid does not match any
    if( !group.belongs( gid ) )
    {
      System.out.println( gid + " does not belong to any user in group file.") ;
      exit(1);
    }
    
    String currentUser = passwd.get( uid );
    String currentGroup = group.get( gid );

	// check the membership card
	/*if( !passwd.membershipVerifier( currentUser , currentGroup) )
	{
		System.out.println( "\"" + currentUser + "\"" +
		  " is not a member of \"" + currentGroup + "\" group");
		exit(1);
	}*/

	// need login password
    if( passwd.passwordVerifier(currentUser) )
    {
      Console console = System.console();
      String pwd = new String( console.readPassword("%s", currentUser + "'s password: ") );
      if( ! pwd.equals(shadow.get(currentUser) ) )
      {
        process.errno = EACCES;
	...
	// need group password
    if(group.passwordVerifier(currentGroup))
    {
      Console console = System.console();
      String pwd = new String(console.readPassword("%s", currentGroup + "'s group password: ") );
      if(!pwd.equals(gshadow.get( currentGroup)))
      {
        process.errno = EACCES;
        perror("Invalid password");
        exit(1);
  ...
  //get the current user umask
  public static short umask() {
  	return process.getUmask();
  }
  //set new umask to user
  public static short umask(short mask) {
	  short previousUmask = process.getUmask() ;
	  process.setUmask(mask) ;
	  return previousUmask ;
  }

  public static int chmod(String path, short mode) throws Exception{
	  FileSystem fileSystem = openFileSystems[ROOT_FILE_SYSTEM];//open the file system
	  IndexNode iNode = new IndexNode();//instanciat an index node
	  short indexNodeNumber = Kernel.findIndexNode(path,iNode);//get the index node
	  //chech the validity of the index node
	  if(indexNodeNumber < 0){
		  process.errno = ENOENT;
		  return -1;
	  }

	  short currentUID = process.getUid();//get the id of the applicant
	  short currentOwner = iNode.getUid();//get the id of the pocessor 
	  
	  //check if the applicant and the pocessor mutch each other
	  if(currentUID != currentOwner && currentUID != 0){
		  process.errno = EPERM;
		  return -1 ;
	  }
	  //check if the mode is valid else throw exception
	  if(mode < (short)0000 || mode > (short)0777)
		  throw new Exception("mode " + Integer.toOctalString(mode) + " is invalid");
	  iNode.setMode((short)(iNode.getMode() & 0xFE00 | mode));//get the current mode and add the new one
	  fileSystem.writeIndexNode(iNode,indexNodeNumber);//write the modified inode in the file system
	  return 0;
  	}
    ...
    short currentUID = process.getUid();//get the id of the applicant
    short currentOwner = iNode.getUid();//get the id of the pocessor
    short currentGID = process.getGid();//get the group id of the applicant
    short groupOwner = iNode.getGid();//get the group id of th pocessor
    //check if the applicant and the pocessor mutch each other
    if(uid != currentOwner){
    	if(currentUID == 0 )//if root
    		iNode.setUid(uid) ;//modify the owner
    	else{
    		process.errno = EPERM ;
    		return -1 ;
    	}
    }
    //check if the applicant group and the pocessor group mutch each other
    if(gid != groupOwner){
    	if(currentUID == 0 || currentUID == currentOwner)//if root
    		iNode.setGid(gid);//modify the group
    	else{
    		process.errno = EPERM ;
    		return -1 ;
    	}
    ...
    fileSystem.writeIndexNode(iNode,indexNodeNumber);//save the modification
    return 0;
  }
